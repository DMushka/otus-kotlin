...
fun ICorChainDsl<GrschbrContext>.stubValidationBadDiameter(title: String) = worker {
    this.title = title
    this.description = """
        Кейс ошибки валидации для диаметр в основной карточки монеты
    """.trimIndent()
    on { stubCase == GrschbrStubs.BAD_DIAMETER && state == GrschbrState.RUNNING }
    handle {
        fail(
            GrschbrError(
                group = "validation",
                code = "validation-diameter",
                field = "diameter",
                message = "Wrong diameter field"
            )
        )
    }
}
