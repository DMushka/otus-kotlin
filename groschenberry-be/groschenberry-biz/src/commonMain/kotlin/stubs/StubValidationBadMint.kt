...
fun ICorChainDsl<GrschbrContext>.stubValidationBadMint(title: String) = worker {
    this.title = title
    this.description = """
        Кейс ошибки валидации для монетного двора в дополнительной карточки монеты
    """.trimIndent()
    on { stubCase == GrschbrStubs.BAD_MINT && state == GrschbrState.RUNNING }
    handle {
        fail(
            GrschbrError(
                group = "validation",
                code = "validation-mint",
                field = "mint",
                message = "Wrong mint field"
            )
        )
    }
}
